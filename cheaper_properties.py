class RealProperty:
    ...
    def __repr__(self):
        return (
            f'RealProperty(rooms = {self.rooms}, square_meters = '
            f'{self.square_meters}, ' + f'price_per_sqm = '
            f'{self.price_per_sqm}, description = {self.description})'
        )


def cheaper_properties(properties: list, reference: RealProperty) -> list:
    return [(pr, pr.price_difference(reference))
            for pr in properties if reference.more_expensive(pr)]
